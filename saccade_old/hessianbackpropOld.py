...
class HessianBackprop(object):
    def __init__(self, layers=[1, 1, 1], use_GPU=False, debug=False, loadW=False,):
        self.loadW = loadW
    ...
    def run_batches(self, inputs, targets, CG_iter=250, init_damping=1.0,
        ...
        for i in range(max_epochs):
            ...
            # print np.float32
            # pprint(self.inputs[0])
            # print np.shape(self.inputs[0])

            # If you are failing this assertion, check the dimensions of your inputs and
            # targets as well. They MUST be uniform across all trials
        ...
        for x in range(len(delta)):
            print >> temp, str(delta_og[x]) + " ---- " + str(self.learning_mask[x])
        ...
        # print 
        Wfile.write(str(self.layers))
        Wfile.write("\n")
        for i in range(len(self.W)):
            Wfile.write(str(self.W[i]))
            Wfile.write("\n")
