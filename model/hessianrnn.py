...
class HessianRNN(HessianBackprop):
    def __init__(self, struc_damping=0.0, **kwargs):
        ...
        # This is where all data for a trial will be stored
    ...
    def drift(self, in_0, rec_0, bias=False, max_iter=100, ret_seq=False,
        ...
        offset if true, assuming this is the first iteration"""

        count = 1

        # offset = self.layers[0]

        # in_0 = x[0:offset]
        # rec_0 = x[offset:]

        rec_1 = util.activate(self, in_0, rec_0, bias=bias)
        rec_t = rec_1
        dX = rec_1 - rec_0

        qval = 0.5*(np.linalg.norm(dX))*(np.linalg.norm(dX))
        oldq = qval
        ...
        while (qval > tol):
            ...
            rec_t = util.activate(self, in_0, rec_1)
            seq.append(rec_t)
    ...
    def calc_grad(self, W=None, inputs=None, targets=None):
        ...
        # print len(range(inputs.shape[1] - 1, -1, -1))
